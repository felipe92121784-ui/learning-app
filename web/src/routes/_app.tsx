...
import { requireActiveUser } from '@/features/auth/auth-guards'
import { useAuth } from '@/features/auth/auth-provider'
import { studentNavigationItems } from '@/features/layout/student-navigation'
...
export const Route = createFileRoute('/_app')({
  beforeLoad: async ({ context, location }) => {
    ...
    return { user: requireActiveUser(user, location.href) }
  },
      ...
      <header className="flex h-16 items-center justify-between gap-4 border-b px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <CardTitle className="truncate">Área do aluno</CardTitle>
            <CardDescription className="hidden truncate sm:block">
              Seu portal de aprendizagem
            </CardDescription>
          </div>
        </div>
